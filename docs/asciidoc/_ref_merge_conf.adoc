= Merge Configuration Fragements
ifdef::env-github[]
:outfilesuffix: .adoc
:!toc-title:
...
image:merge-fragments.drw.png[Merge Configuration Fragements]

[NOTE]
====
In the above example, the `_env` lookup function is used to retrieve the value from an environment variable.
This function is provided as a built-in function by the YAML-ES utility.
...
To merge configuration fragments, use the `merge config` command of the YAML-ES utilities.

[source, shell]
----
yamlesutils.sh merge config --help
Usage: yamlesutils merge config [-hV] [-c=FILE]... [-d=DIR]...
                                [--lookup-functions=FILE]... (-o=FILE |
                                [--project=DIR | --ignore-missing-values])
Merge configuration fragments for YAML entity store.
  -c, --config=FILE   Configuration fragment.
  -d, --dir=DIR       Directory to scan for YAML configuration fragments.
  -h, --help          Show this help message and exit.
      --ignore-missing-values
                      Ignore missing configuration of values (only for
                        projects).
      --lookup-functions=FILE
                      configure lookup functions.
  -o, --output=FILE   File to write generated YAML configuration.
...
Following global options can be applied before the command.

.Example: Global Options
[source, shell]
----
yamlesutils.sh --audit=audit.log \ #<1>
  merge config -c fragment.yaml -o values.yaml #<2>
----
<1> Global option to activate audit log file.
<2> Merge command and its options
...
If not specified the audit logs are only written to `stderr`.
|===
...
yamlesutils.sh merge config \ #<1>
  --config=fragments/global.yaml \ #<2>
  --config=fragment/local.yaml \ #<3>
  -o - #<4>
----
...
To use the custom function, the a provider for this function must be defined.
...
----
<1> Defines an alias `users` for the lookup function configuration.
...
yamlesutils.sh merge config \ #<1>
  ...
  -c fragments/global.yaml -c fragments/local.yaml -c fragments/users.yaml \ #<3>
  -o - #<4>
----
<1> Command to merge configuration fragments.
