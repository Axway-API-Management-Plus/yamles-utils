...
|*Synopsis*
|Retrieves values from AWS Secrets Manager.
...
The lookup provider uses AWS SDK to access the secrets in the AWS Secrets Manager.
...
In this example the secret `sandbox/anm` is stored in AWS Secrets Manager.
...
The secret can also be represented as plain text, which is a JSON document storing the values with the secret key as property name.
